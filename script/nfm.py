...
parser.add_argument('--model_dir', default = '../model/nfm', type = str, help = 'model dir')
parser.add_argument('--server_model_dir', default = '../model/nfm/export', type = str, help = 'server model dir')
...
#deep-fm model
def nfm_rec(features, labels, mode, params):
    ##################################################
    ...
    embeddings_bi_inter = tf.einsum('nik,njk->nijk', embeddings, embeddings)
    print(embeddings_bi_inter.get_shape())
    embeddings_bi_inter = tf.reduce_sum(embeddings_bi_inter, [1, 2])
    embeddings_bi_inter = embeddings_bi_inter / 2
    print(embeddings_bi_inter.get_shape())

    #create fully connected layers, perform weight decay, batch normlization, drop_out with each hidden layer
    net = tf.reshape(embeddings_bi_inter, shape = [-1, params['embedding_size']])
    print(net.get_shape())
    hidden_index = 0
