...
parser.add_argument('--model_dir', default = '../model/pnn', type = str, help = 'model dir')
parser.add_argument('--server_model_dir', default = '../model/pnn/export', type = str, help = 'server model dir')
...
parser.add_argument('--hidden_units', default = '26_56_56_36', type = str, help = 'hidden units number of each layer')
parser.add_argument('--embedding_size', default = 2, type = int, help = 'embedding size')
...
parser.add_argument('--product_type', default = 'in_product', type = str, help = 'in_product, out_product')
...
def pnn_rec(features, labels, mode, params):
    ...
    #outer product pnn
        ...
        embeddings_p = tf.reshape(embeddings_v, [-1, params['field_size'] * params['embedding_size']])
        embeddings_r = tf.reshape(tf.einsum('ni,nj->nij', embeddings_p, embeddings_p), [-1, params['field_size'], params['embedding_size'], params['field_size'], params['embedding_size']])
        embeddings_r = tf.transpose(embeddings_r, perm = [0, 1, 3, 2, 4])
        embeddings_r = tf.reshape(embeddings_r, [-1, params['field_size'], params['field_size'], params['embedding_size'] * params['embedding_size']])

        feat_arr = []
        for i in range(params['field_size'] - 1):
            row_len = params['field_size'] - 1 - i
            row = tf.slice(embeddings_r, [0, i, i + 1, 0], [-1, 1, row_len, -1])
            feat = tf.reshape(row, [-1, row_len * params['embedding_size'] * params['embedding_size']])
            feat_arr.append(feat)
...
def main(argv):
    ...
    # build pnn recommanding estimator
